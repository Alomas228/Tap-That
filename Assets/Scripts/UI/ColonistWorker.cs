...
public class ColonistWorker : MonoBehaviour
{
    ...
    {
        // Для обычных рабочих - стандартная логика
        if (resourceManager == null || !HasResourcesToDeliver()) return;

        // Если это исследователь - его логика доставки в WorkCycle
        if (this is ResearchStationWorker)
        {
            // Исследователи обрабатывают доставку в своем WorkCycle
            return;
        }

        // Стандартная логика для других рабочих
